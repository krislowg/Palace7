...
public class PaymentAccount {
    ...
    @FXML
    private TextField ccvCode;

    @FXML
    private TextField coupCode;
    ...
    @FXML
    private ChoiceBox<String> expMonth;

    @FXML
    private ChoiceBox<Integer> expYear;

    private ObservableList<String> cCType = FXCollections.observableArrayList("MasterCard", "Visa",
            "AmericanExpress", "Discovery");

    private ObservableList<String> expMon = FXCollections.observableArrayList("January", "February",
    ...
    void showConfirmation(ActionEvent event) throws IOException {
        ...
        try {
            PreparedStatement ps = connection.prepareStatement("INSERT INTO GUEST (EMAIL, NAME, LASTNAME, NOPEOPLE, NOROOMS, CHECKIN, CHECKOUT, ROOMTYPE," +
                    " PASSWORD) VALUES ('"
                    + email
                    + "', '"
                    + fName
        ...
        GuestReservation finalDataSet = new GuestReservation(email,fName,lName,
                Integer.parseInt(roomChoice.getNoPeople()),roomchk,roomChoice.getCheckIn().toString(),
                        roomChoice.getCheckOut().toString(),roomChoice.getRoomType(),password);
